...
- **Structural** — **before writing anything, `grep` the project for an existing usage of the Gofive target** (blocks, demos, other pages, the component's own docs page) and **mirror its layout/composition** — when the repo already shows the idiomatic shell, copy it; don't improvise a generic one. Then perform the rewrite (e.g. convert a composable `<Select><SelectItem/></Select>` into Gofive's `<Select options={[…]} />`). When a prop/behavior has no faithful mapping, **do not guess** — keep the closest equivalent and leave a `// TODO(gofive-migrate): <what to check>` comment, then flag it in the report.
